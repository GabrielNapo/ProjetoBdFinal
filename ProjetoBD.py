...
EMAIL_REMETENTE = "g.luiz.6128@gmail.com"
SENHA_REMETENTE = "fbpf jmtm okcn hsgv"
...
def cartaoDados():
    ...
    def verificar_campos_vazios():
        if not entry_nome.get().strip() or not entry_cartao.get().strip() or not entry_cvv.get().strip() or not entry_expiry.get().strip():
            tkinter.messagebox.showerror("Erro", "Todos os campos devem ser preenchidos.")
        else:
            cartaoSubmit()
...
def comprinhasTela():
    ...
    label_descricao = customtkinter.CTkLabel(app, text=("Lhamborguini"), font=("Arial", 16))
    label_explica = customtkinter.CTkLabel(app,text=("Somente um por usúario\nPreço: R$ 100.000.000,99"),font=("Arial",12))
    ...
    botao_historico = customtkinter.CTkButton(app, text="Histórico de Compras", command=historicoCompras)
    botao_historico.grid(row=2, column=1, padx=10, pady=(10,0), sticky="w")
    voltarLogin = customtkinter.CTkButton(app,text="VOLTAR",font=("Arial",12),command=loginDados)
    voltarLogin.grid(row=0,column=0,padx=(0,50),pady=(0,0),sticky="e")
    ...
    if (sucesso == True):
        ...
        label_sucesso.grid(row=1, column=1, padx=10, pady=(100, 0), sticky="w")
     
    else:
 
        botao_comprar = customtkinter.CTkButton(app, text="Comprar", width=200, command=escolherCartao)
        botao_comprar.grid(row=1, column=1, padx=10, pady=(100, 0), sticky="w")
...
def confirmarCompra(janela):
    salvarCompra("Lhamborguini", "R$ 100.000.000,99")
    janela.destroy()
